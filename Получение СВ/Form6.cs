    ...
    public partial class Form6 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                double X = m + σ * (N - 6);   //формула для получения случайного нормально распределенного числа с заданными параметрами m и σ
                X = Math.Round(X, 3);
                listBox1.Items.Add(X.ToString());
                x[i] = X;   //Присвоение значения X
                //Вычисление значения функции в точке X
                F[i] = 1 / (σ * Math.Sqrt(2 * Math.PI)) * Math.Exp(-(1 / 2) * Math.Pow((X - m) / σ, 2));
